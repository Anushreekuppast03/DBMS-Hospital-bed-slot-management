...
from werkzeug.security import check_password_hash  # Assuming you're using Werkzeug for password hashing
...
def signup():
    if request.method=="POST":
        ...
        date=User.query.filter_by(dob=dob).first()
...
def login():
    if request.method == "POST":
        ...
        dob = request.form.get('dob')
        user = User.query.filter_by(srfid=srfid).first()
        date = User.query.filter_by(dob=dob)
        if user and date:  # Assuming `user.dob` is hashed password
            login_user(user)
        ...
        # Handle GET request for the login page (if needed)
        return render_template("userlogin.html")
...
def slotbooking():
    ...
    if request.method == "POST":
        ...
        # Decrease the bed count based on the bed type
        if bedtype == "NormalBed":       
            seat = check2.normalbed
            if seat > 0:
                check2.normalbed = seat - 1
            else:
                flash("No more Normal Beds available", "danger")
                return render_template("booking.html", query=query, query1=query1)
            ...
            if seat > 0:
                ...
                flash("No more HICU Beds available", "danger")
                return render_template("booking.html", query=query, query1=query1)
            ...
            if seat > 0:
                ...
                flash("No more ICU Beds available", "danger")
                return render_template("booking.html", query=query, query1=query1)
            ...
            if seat > 0:
                ...
                flash("No more Ventilator Beds available", "danger")
                return render_template("booking.html", query=query, query1=query1)
